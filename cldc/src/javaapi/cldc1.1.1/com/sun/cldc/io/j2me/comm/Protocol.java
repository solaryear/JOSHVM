...
public class Protocol extends BufferedConnectionAdapter 
    ...
    protected int nonBufferedRead(byte b[], int off, int len)
        ...
        for (;;) {
            try {
                bytesRead = native_readBytes(handle, b, off, len, blocking);
            } finally {
                if (iStreams == 0) {
    ...
    protected int readBytesNonBlocking(byte b[], int off, int len)
        ...
        try {
            ...
            bytesRead = native_readBytes(handle, b, off, len, false);
        } finally {
            if (iStreams == 0) {
                throw new InterruptedIOException("Stream closed");
            }
        ...
        if (bytesRead < 0) {
			//Since it's non-blocking read, bytesRead == 0, which means nothing to read, is possible
            eof = true;
        }
    ...
    private static native int native_readBytes(int hPort, byte b[], int off,
        int len, boolean blocking) throws IOException;
