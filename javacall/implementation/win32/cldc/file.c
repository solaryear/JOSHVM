...
javacall_result javacall_file_init(void) {
	DEBUG_PRINT("javacall_file_init\n");
    return JAVACALL_OK;
}
...
javacall_result javacall_file_finalize(void) {
	DEBUG_PRINT("javacall_file_finalize\n");
...
javacall_result javacall_file_open(const javacall_utf16*  unicodeFileName,
    ...
    int oFlag =  O_BINARY;
    int creationMode = 0;
	DEBUG_PRINT("javacall_file_open\n");

    wchar_t wOsFilename[MAX_NULL_TERMINATED_NAME_LENGTH]; // max file name

    if( fileNameLen > JAVACALL_MAX_FILE_NAME_LENGTH ) {
        javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FILE, "javacall_file_open: File name length exceeds max file length");
...
javacall_result javacall_file_flush(javacall_handle handle) {
	DEBUG_PRINT("javacall_file_flush\n");
...
javacall_result javacall_file_delete(const javacall_utf16* unicodeFileName,
                                     int fileNameLen){
	DEBUG_PRINT("javacall_file_delete\n");
    ...
    if( fileNameLen > JAVACALL_MAX_FILE_NAME_LENGTH ) {
        javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FILE, "javacall_file_delete: File name length exceeds max file length");
        return JAVACALL_FAIL;
    }
...
javacall_result javacall_file_truncate(javacall_handle handle, javacall_int64 size){
    ...
    int res;
	DEBUG_PRINT("javacall_file_truncate\n");
...
javacall_int64 javacall_file_seek(javacall_handle handle,
	...
	DEBUG_PRINT("javacall_file_seek\n");
...
javacall_int64 javacall_file_sizeofopenfile(javacall_handle handle){
	...
	DEBUG_PRINT("javacall_file_sizeofopenfile\n");
...
javacall_int64 javacall_file_sizeof(const javacall_utf16* fileName,
    ...
    struct _stat stat_buf;
    int result;
	DEBUG_PRINT("javacall_file_sizeof\n");

    wchar_t wOsFilename[MAX_NULL_TERMINATED_NAME_LENGTH]; // max file name

    if( fileNameLen > JAVACALL_MAX_FILE_NAME_LENGTH ) {
        javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FILE, "javacall_file_sizeof: File name length exceeds max file length");
        return -1;
    }
...
javacall_result javacall_file_exist(const javacall_utf16* fileName,
    ...
    struct _stat stat_buf;
    int res;
	DEBUG_PRINT("javacall_file_exist\n");

    wchar_t wOsFilename[MAX_NULL_TERMINATED_NAME_LENGTH]; // max file name

    if( fileNameLen > JAVACALL_MAX_FILE_NAME_LENGTH ) {
        javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FILE, "javacall_file_exist: File name length exceeds max file length");
        return JAVACALL_FAIL;
    }
...
javacall_result javacall_file_rename(const javacall_utf16*  unicodeOldFilename,
     ...
     int res;
	 DEBUG_PRINT("javacall_file_rename\n");
    ...
    if( oldNameLen > JAVACALL_MAX_FILE_NAME_LENGTH ) {
        javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FILE, "javacall_file_rename: Old file name length exceeds max file length");
        return JAVACALL_FAIL;
    }
    if( newNameLen > JAVACALL_MAX_FILE_NAME_LENGTH ) {
        javacall_logging_printf(JAVACALL_LOGGING_ERROR, JC_FILE, "javacall_file_rename: New file name length exceeds max file length");
        return JAVACALL_FAIL;
    }
