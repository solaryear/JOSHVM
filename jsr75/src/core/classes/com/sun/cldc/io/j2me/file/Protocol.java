...
import org.joshvm.security.internal.Permissions;
...
public class Protocol extends ConnectionBaseAdapter implements FileConnection {
	...
	/** Root name of internal appdb root path */
	private String appdb_root;
    ...
    public Protocol() {
        ...
        fileHandler = null;
		securityToken = null;
		appdb_root = System.getProperty("org.joshvm.ams.appdbpath.root");
		if (appdb_root != null) {
			appdb_root = "file:///" + appdb_root;
		}
    }
     ...
     */
    public Connection openPrim(SecurityToken token, String name, int mode, boolean timeouts)
            throws IOException {
        securityToken = token;
    ...
    private final void checkReadPermission(String fileURL, int mode)
        ...
        if ((appdb_root != null) && fileURL.startsWith(appdb_root)) {
			if (securityToken == null) {
				throw new SecurityException("No read permission to "+fileURL);
			}
     ...
     * @throws IllegalModeException if connection is write only
     */
    protected final void checkReadPermission() throws SecurityException, InterruptedIOException {
        checkReadPermission(fileURL, mode);
    }
    ...
    protected final void checkRootReadPermission()
            throws SecurityException, InterruptedIOException {
		if (securityToken == null) {
			throw new SecurityException("No read permission to root");
		}
        securityToken.checkIfPermissionAllowed(Permissions.AMS);
		return;
    }
    ...
    private final void checkWritePermission(String fileURL, int mode)
        ...
        if ((appdb_root != null) && fileURL.startsWith(appdb_root)) {
			if (securityToken == null) {
				throw new SecurityException("No write permission to "+fileURL);
			}
			securityToken.checkIfPermissionAllowed(Permissions.AMS);
        }
		return;
    ...
    private static BaseFileHandler getFileHandler() {
        String def = "com.sun.cldc.io.j2me.file.DefaultFileHandler";
        String n = null;
        try {
