...
#if ENABLE_DEBUG
#define DEBUG_PRINT(s)          REPORT_INFO(LC_FC, s);
#define DEBUG_PRINT1(s, a1)     REPORT_INFO1(LC_FC, s, a1);
#define DEBUG_PRINT2(s, a1, a2) REPORT_INFO2(LC_FC, s, a1, a2);
#else
...
static int get_pcsl_string(jobject stringHandle, pcsl_string ** string)
{
    ...
    {
        KNI_ThrowNew(KNINullPointerException, EXCEPTION_MSG(fcHandleIsNull));
        return -1;
    }
    else
    {
 ...
 */
typedef javacall_result (*property_func)(javacall_utf16 *, int, javacall_bool);
...
static char* get_property(char** var, int maxLength, property_func get_prop, int useFilePrefix) {
    ...
    if (JAVACALL_OK != get_prop(value, maxLength, JAVACALL_FALSE)) {
        pcsl_mem_free(value);
        return NULL;
    }
...
Java_com_sun_cldc_io_j2me_file_DefaultFileHandler_directorySize()
{
    ...
    else
    {
        ...
        if (PCSL_STRING_PARAMETER_ERROR(fileName)) {
            ...
            if (JAVACALL_OK != javacall_fileconnection_dir_content_size(fileName_data, recurse, &res))
            {
                res = (jlong)(-1);
            }
        }
...
Java_com_sun_cldc_io_j2me_file_DefaultFileHandler_mkdir()
{
    ...
    if (NULL == fileName)
    {
        KNI_ThrowNew(KNINullPointerException, EXCEPTION_MSG(fcDirNameIsNull));
    }
    else
    {
        GET_PCSL_STRING_DATA_AND_LENGTH(fileName)
...
Java_com_sun_cldc_io_j2me_file_DefaultFileHandler_openDir()
{
    ...
    else
    {
        ...
        if (PCSL_STRING_PARAMETER_ERROR(fileName)) {
            ...
            if (NULL == fileList)
            {
                //KNI_ThrowNew(KNINullPointerException, EXCEPTION_MSG(fcDirOpenFailed));
                res = (jlong)-1;
            }
            else
            {
...
Java_com_sun_cldc_io_j2me_file_DefaultFileHandler_dirGetNextFile()
{
    ...
    else
    {
        while (!finish)
        {
        	javacall_bool isHidden = JAVACALL_FALSE, isDirectory = JAVACALL_FALSE;
            found_data = (jchar *)javacall_dir_get_next(fileList, &found_len, &isHidden, &isDirectory);
            if (NULL != found_data)
            {
                /* add path prefix */
                ...
                if (PCSL_STRING_OK != pcsl_string_append(pLongName, fileName)
                 ...
                 || PCSL_STRING_OK != pcsl_string_append_buf(pLongName, found_data, found_len)) {
                    ...
                    if (PCSL_STRING_PARAMETER_ERROR(pLongName)) {
                        ...
                        if (JAVACALL_OK == javacall_fileconnection_is_hidden(pLongName_data, pLongName_len, &isHidden))
                        {
                            if ((JAVACALL_FALSE == isHidden) || (JAVACALL_TRUE == includeHidden))
                            {
    ...
    if (0 == convertStatus)
    {
        ...
        else
        {
            ...
            if (PCSL_STRING_PARAMETER_ERROR(mroot)) {
                ...
                if (JAVACALL_OK != javacall_fileconnection_get_path_for_root(mroot_data, path, JAVACALL_MAX_ROOT_PATH_LENGTH))
                {
                    KNI_ReleaseHandle(returnValueHandle);
                }
                else
...
Java_com_sun_cldc_io_j2me_file_DefaultFileHandler_lastModified()
{
    ...
    else
    {
        ...
        if (PCSL_STRING_PARAMETER_ERROR(fileName)) {
            ...
            if (JAVACALL_OK == javacall_fileconnection_get_last_modified(fileName_data, &res))
            {
...
Java_com_sun_cldc_io_j2me_file_DefaultFileHandler_read()
{
    ...
    {
        KNI_ThrowNew(KNINullPointerException, EXCEPTION_MSG(fcFileClosed));
    }
...
Java_com_sun_cldc_io_j2me_file_DefaultFileHandler_write()
{
    ...
    if (NULL != handle)
    {
        ...
        if (NULL == data)
        {
			DEBUG_PRINT("File.write() << Error KNIOutOfMemoryError\n");
            KNI_ThrowNew(KNIOutOfMemoryError, NULL);
        }
        else
        {
    ...
    {
        DEBUG_PRINT("File.write() << Error KNINullPointerException\n");
        KNI_ThrowNew(KNINullPointerException, EXCEPTION_MSG(fcFileClosed));
    }
...
Java_com_sun_cldc_io_j2me_file_DefaultFileHandler_positionForWrite()
{
    ...
    else
    {
        ...
        KNI_ThrowNew(KNIIOException, EXCEPTION_MSG(fcFileClosed));
    }
...
#define GET_FIELDID(id, fieldName, fieldType)                   \
    ...
    {                                                           \
        ...
        if (id == 0) {                                          \
            KNI_ThrowNew(KNINullPointerException,              \
